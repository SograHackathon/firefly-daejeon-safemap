...
export default function SearchPanel(props: Props) {
  ...
  // 화면 모드
  // 'search'    = 검색바 (디폴트)
  // 'placeCard' = 검색결과 클릭 → 핀+카드+출발/도착 버튼
  // 'directions'= 길찾기 모드 (출/도착 양쪽 입력 패널)
  // 'route'     = 경로 후보 표시
  const mode: 'search' | 'placeCard' | 'directions' | 'route' =
    routeResult ? 'route'
    : (start && end) ? 'directions'    // 둘 다 정해진 직후 routing 호출 전 잠시
    : (start || end) ? 'directions'    // 한쪽만 정해진 상태 = 길찾기 패널
        ...
        <>
          ...
          <div className="absolute top-3 left-3 right-3 z-30 bg-white rounded-2xl shadow-lg">
            <div className="flex items-start">
              ...
              <div className="flex-1 min-w-0 py-1">
                <button
                  onClick={() => { onRouteResult(null); setEditingField('start'); setQ(''); setPlaces([]); }}
                  className="w-full flex items-center gap-2 px-2 py-2.5 text-left active:bg-gray-50 rounded-lg"
                >
                  <span className="w-2.5 h-2.5 rounded-full bg-blue-500 flex-shrink-0"></span>
                  <span className="text-[11px] text-gray-400 font-bold flex-shrink-0">출발지</span>
                ...
                <div className="border-t border-gray-100 mx-2" />
                <button
                  onClick={() => { onRouteResult(null); setEditingField('end'); setQ(''); setPlaces([]); }}
                  className="w-full flex items-center gap-2 px-2 py-2.5 text-left active:bg-gray-50 rounded-lg"
                >
                  <span className="w-2.5 h-2.5 rounded-full bg-red-500 flex-shrink-0"></span>
                  <span className="text-[11px] text-gray-400 font-bold flex-shrink-0">도착지</span>
          ...
          {/* 경로 카드 */}
          <div className="absolute bottom-0 left-0 right-0 z-30 bg-white rounded-t-3xl px-4 pt-3 pb-5 shadow-2xl max-h-[60vh] overflow-y-auto" style={{ paddingBottom: 'max(20px, env(safe-area-inset-bottom))' }}>
            <div className="w-9 h-1 bg-gray-300 rounded-full mx-auto mb-3" />
            ...
            {routeResult && (
              <>
                ...
                {selectedCandidate && (
                  <button className="w-full mt-3 py-3.5 bg-gray-900 text-white rounded-2xl font-bold text-sm active:bg-gray-700">
                    안내 시작
                  </button>
                )}
              </>
